        ...
        MysqlMerpati.query(`SELECT * FROM playerucp WHERE DiscordID = '${userid}'`, async (error, row) => {
            if (row[0]) {
                ...
                const msgEmbed = new MessageEmbed()
                    .setAuthor({ name: "Indo Kita Roleplay", iconURL: client.config.ICON_URL })
            ...
            else return IntError(interaction, "Anda tidak memiliki akun terdaftar di dalam server Kota Bersama Roleplay!");
        })
    }
}