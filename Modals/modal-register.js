    ...
    run: async (interaction) => {
        ...
        MysqlMerpati.query(`SELECT * FROM playerucp WHERE ucp = '${inputName}'`, async(err, row) =>{
            if(row.length < 1) {
                ...
                .setDescription(`Hallo **${inputName}**!! Selamat akun UCP Anda telah berhasil terdaftar di Kota Indo Kita Silahkan Gunakan Code/Pin Verifikasi dibawah ini untuk pendaftaran lebih lanjut di dalam ingame Anda, **Selamat Beroleplay!**.\n\n- Data Akun User Control Panel -\n• \`Pemilik Akun\`: ${userid} (\`${interaction.user.tag}\`)\n• \`Nama Akun\`: ${inputName}\n• \`Pin Akun\`: ${randCode}\n• \`Waktu Pendaftaran\`: <t:${Math.round(Date.now() / 1000)}:R>`)
                .setColor("GOLD")
